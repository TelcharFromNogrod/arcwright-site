...
for (const col of [
  'ALTER TABLE payments ADD COLUMN download_token TEXT',
  'ALTER TABLE payments ADD COLUMN amount_crypto REAL',
]) {
...
function createPayment({ id, email, productSlug, amountUsd, amountCrypto, crypto, chain, payAddress, addressIndex }) {
  db.prepare(`
    INSERT INTO payments (id, email, product_slug, amount_usd, amount_crypto, crypto, chain, pay_address, address_index, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, email, productSlug, amountUsd, amountCrypto || null, crypto, chain, payAddress, addressIndex, Date.now());
}
